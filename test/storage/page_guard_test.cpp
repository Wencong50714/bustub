...
TEST(PageGuardTest, BasicTest) {
  ...
  // test BasicPageGuard(ReadPageGuard &&that)
  {
    auto basic_guard = bpm->FetchPageBasic(page_id_temp);
    EXPECT_EQ(2, page0->GetPinCount());
    auto basic_guard_2 = BasicPageGuard(std::move(basic_guard));
    EXPECT_EQ(2, page0->GetPinCount());
  }
  // test ~BasicPageGuard()
  EXPECT_EQ(1, page0->GetPinCount());
  ...
  // test BasicPageGuard::operator=(BasicPageGuard &&that)
  {
    ...
    auto basic_guard_2 = bpm->FetchPageBasic(page_id_temp);
    EXPECT_EQ(3, page0->GetPinCount());
    basic_guard_1 = std::move(basic_guard_2);
    // ! The original resource of basic_guard_1 has been destroyed
    EXPECT_EQ(2, page0->GetPinCount());
  }
  EXPECT_EQ(1, page0->GetPinCount());
  ...
  // test BasicPageGuard::Drop()
  {
    ...
    auto basic_guard_2 = bpm->FetchPageBasic(page_id_temp);
    EXPECT_EQ(3, page0->GetPinCount());
    auto basic_guard_3 = std::move(basic_guard_1);
    EXPECT_EQ(3, page0->GetPinCount());
    basic_guard_1.Drop();
    EXPECT_EQ(3, page0->GetPinCount());
    basic_guard_3.Drop();
